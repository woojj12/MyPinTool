...
//	fprintf(stdout, "%lx malloc after %p\n", threadid, ptr);

//	if(ptr == NULL)
//		return NULL;
//
//	PIN_MutexLock(&malloc_lock);
//	malloc_root = InsertMAddress(malloc_root, (ADDRINT)ptr, size);
...
////	treeNode *node;
//
//	reptr = (void*)realloc(ptr, size);
////	fprintf(stdout, "%lx realloc after %p\n", threadid, reptr);
//
////	if(reptr == NULL)
////		return NULL;
////
...
////	{
////		PIN_MutexLock(&malloc_lock);
////		malloc_root = InsertMAddress(malloc_root, (ADDRINT)reptr, size);
////		PIN_MutexUnlock(&malloc_lock);
////
////	}
////	else if(reptr == _ptr)
////	{
////		PIN_MutexLock(&malloc_lock);
////		node = FindAddress(malloc_root, (ADDRINT)_ptr);
////		if(node == NULL)
////			assert(0);
////		node->size = size;
////		PIN_MutexUnlock(&malloc_lock);
...
////	{
////		PIN_MutexLock(&malloc_lock);
////		malloc_root = DeleteAddress(malloc_root, (ADDRINT)_ptr);
////		malloc_root = InsertMAddress(malloc_root, (ADDRINT)reptr, size);
////		PIN_MutexUnlock(&malloc_lock);
////	}
//
...
VOID Image(IMG img, VOID *v)
{
	...
	{
		RTN_Open(mallocRtn);
		RTN_Replace(mallocRtn, AFUNPTR(My_Malloc));
		RTN_Close(mallocRtn);
//		RTN_Open(mallocRtn);
//
...
//	{
//		RTN_Replace(reallocRtn, AFUNPTR(My_Realloc));
//		RTN_Open(reallocRtn);
//
...
//	{
//		RTN_Replace(freeRtn, AFUNPTR(My_Free));
////		RTN_Open(freeRtn);
